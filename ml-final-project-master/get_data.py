...
def get_outstanding_shares(ticker_symbol):
    ...
    company_url = a_tags[0].get_attribute("href")
    driver.get(company_url)
    outstanding_shares_url = driver.current_url.replace('marketcap/', 'shares-outstanding/')
...
"""
def get_sector_data(sectors=None, run_share_price=True, run_outstanding=True, run_market_cap=True):
    ...
    for sector in sectors:
        ...
        for index, row in companies_in_sector_df.iterrows():
            try:
                ...
                print("\t* Failed to process company: " + ticker_symbol)
                failed.append(ticker_symbol + " | " + company_name)
                continue
...
def main():
    get_sector_data(sectors=None, run_share_price=True, run_market_cap=False, run_outstanding=True)
